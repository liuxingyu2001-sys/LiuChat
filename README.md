...
| 公屏 AI 聊天 | AI 像真实玩家一样参与公共聊天：公屏点名（@AI 或提到它的名字）就回复，回答以它的玩家名走正常聊天渲染（提及高亮、`/liuc ignore` 屏蔽、聊天日志、跨服同步都生效），并把最近公屏消息作为聊天氛围上下文；可配概率插话、冷却、上下文条数；需开 `ai.enable` 与 `ai.assistant.enable`，另有 `ai.chat` 段开关 |
